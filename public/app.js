...
// Allow pressing Enter in custom TLD input
...
async function generateDomains() {
    ...
    if (useLLM) progressText.textContent = '🤖 Asking AI to generate domains... (may take 10-30s)';
    else        progressText.textContent = '⚡ Building keyword-based domain names...';
...
async function addSale() {
    ...
    try {
        ...
        alert('✅ Sale added!'); loadSales(); refreshStats();
...
function displayPortfolio(portfolio) {
    ...
    if (!portfolio || portfolio.length === 0) { container.innerHTML = '<div class="empty-state"><i class="fas fa-briefcase"></i><p>Portfolio is empty</p></div>'; return; }
...
async function addToPortfolio() {
    ...
    try {
        ...
        alert('✅ Added to portfolio!'); loadPortfolio();
...
async function saveConfig() {
    ...
    try { await fetch(`${API}/config`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(cfg) }); alert('✅ Saved!'); } catch { alert('❌ Save failed'); }
}
